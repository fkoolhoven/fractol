...
/*   Updated: 2023/03/12 18:39:13 by fkoolhov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_fractol	set_parameters_julia(int argc, char **argv)
{
	...
	if (argc != 4)
	{
		ft_putendl_fd("Error: Incorrect number of arguments.\nInput should be: [./fractol] [julia] [real component of complex number] [imaginary component of complex number]\nFor example: ./fractol julia 0.4 0.2", 2);
		exit (EXIT_FAILURE);
	}
	fractol.julia = 1;
	fractol.c_real = ft_atof(argv[2]);
	fractol.c_imaginary = ft_atof(argv[3]);
	if (fractol.c_real == -42 || fractol.c_imaginary == -42 || fractol.c_real < -2.0 || fractol.c_real > 2.0 || fractol.c_imaginary < -2.0 || fractol.c_imaginary > 2.0)
	{
		printf("real = %f\nimag = %f\n", fractol.c_real, fractol.c_imaginary);
		ft_putendl_fd("Error: Julia parameters must be fractional numbers between -2.0 and 2.0. No spaces allowed.\nFor example: ./fractol julia 0.4 0.2", 2);
		exit (EXIT_FAILURE);
	}
